...
export const getOutlinedFeature = (features: Feature<Polygon | MultiPolygon, Properties>[]) => {
  ...
  return simplify(polygonToLine(masked), { tolerance: SIMPLIFY_TOLERANCE, highQuality: false });
};
...
export const area = (geometry: AllGeoJSON) => {
  return turf.area(
    geometry as unknown as Feature<any> | FeatureCollection<any> | turf.helpers.Geometry
  );
};
...
export const getGapsByConvexHull = (polygonsData: { [x: string]: PolygonPartRecordModelType }) => {
  const polygonsCoordinates = Object.values(polygonsData).map(
    (poly: PolygonPartRecordModelType) => poly.footprint.coordinates
  );
  const turfPolygons = polygonsCoordinates.map((coordinate) => turf.polygon(coordinate));
