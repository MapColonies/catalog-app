...
export const useHeightFromTerrain = (options?: UseHeightFromTerrainProps): IHeightFromTerrain => {
  ...
  useEffect(() => {
    if (
      options &&
      (!prevPositions.current || !isArrayEqual(prevPositions.current, options.position))
    ) {
    ...
    if (coordinates) {
      ...
      if (is2dArr) {
        ...
        cartographicArr.forEach((ring: any): void => {
          // @ts-ignore
          promises.push(
            cesiumSampleTerrainMostDetailed(mapViewer.terrainProvider, ring as CesiumCartographic[])
          );
        });
