  ...
  ({ layer, isOpen, onSetOpen, onPublish }) => {
      ...
      return intl.formatMessage(
        { id: 'publish-unpublish.dialog.publish.message' },
        {
          action: emphasizeByHTML(
            `${intl.formatMessage({ id: 'publish-unpublish.dialog.publish.action' })}`
          ),
        }
      );
    }, []);
    ...
    const unpublishMessage = useMemo((): string => {
      return intl.formatMessage(
        ...
        {
          action: emphasizeByHTML(
            `${intl.formatMessage({ id: 'publish-unpublish.dialog.unpublish.action' })}`
          ),
        }
      );
    }, []);
    ...
    useEffect(() => {
      if (
        ...
        (mutationQuery.data as { updateStatus: string } | undefined)?.updateStatus === 'ok'
      ) {
        onSetOpen(false);
        dispatchAction({
          ...
          <DialogContent className="dialogBody">
            ...
            <Typography
              ...
              dangerouslySetInnerHTML={{
                __html: isUnpublished(layer as any) ? publishMessage : unpublishMessage,
              }}
            ></Typography>
          </DialogContent>
          <DialogActions>
