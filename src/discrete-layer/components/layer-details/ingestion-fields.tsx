...
const FileItem: React.FC<{ file: FileData }> = ({ file }) => {
  return (
    <>
      ...
      <Box style={{ direction: 'ltr' }}>{defaultFormatters.formatFileSize(null, file)}</Box>
    </>
  );
};
      ...
      if (!isEmpty(queryResolveMetadataAsModel.error) || !isEmpty(chosenMetadataError)) {
        if (reloadFormMetadata) {
          reloadFormMetadata(
            ...
            {
              recordModel: {},
              error: chosenMetadataError ?? (queryResolveMetadataAsModel.error as unknown),
            } as MetadataFile
          );
        }
      }
    ...
    useEffect(() => {
      setIsImportDisabled(
        !selection.files.length ||
          ...
          isError ||
          (!queryValidateSource.loading &&
            queryValidateSource.data?.validateSource[ONLY_ONE_SOURCE].isValid === false)
      );
              ...
              },
              {
                recordModel: {
                  ...
                  resolutionDegreeMaxValue:
                    queryValidateSource.data?.validateSource[ONLY_ONE_SOURCE]['resolutionDegree'],
                },
                ...
                <Button
                  ...
                  onClick={(): void => {
                    importJSONFileFromClient((e) => {
                      const resultFromFile = JSON.parse(e.target?.result as string) as Record<
                        string,
                        unknown
                      >;
