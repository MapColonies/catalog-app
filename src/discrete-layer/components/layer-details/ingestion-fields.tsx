            ...
            <Box className="detailsFieldValue">
              ...
              {index === FILES && values[index] !== '' && (
                <Box className="filesList">
                  {selection.files.map(
                    (file: FileData, idx: number): JSX.Element | undefined => {
                      if (
                        idx < NUM_OF_ROWS - 1 ||
                      ...
                      if (
                        ...
                        return (
                          <MoreItem key={file.id} files={selection.files} />
                        );
                      }
                    }
                  )}
...
export const IngestionFields: React.FC<
  PropsWithChildren<IngestionFieldsProps>
> = observer(
  ({
    recordType,
    fields,
    ...
    const [isFilePickerDialogOpen, setFilePickerDialogOpen] =
    ...
    const [chosenMetadataFile, setChosenMetadataFile] = useState<string | null>(
      null
    );
    const [chosenMetadataError, setChosenMetadataError] = useState<{
      response: { errors: { message: string }[] };
    } | null>(null);

    const queryResolveMetadataAsModel = useQuery<{
      resolveMetadataAsModel: LayerMetadataMixedUnion;
    }>();
    const queryValidateSource = useQuery<{
      validateSource: SourceValidationModelType[];
    }>();
    ...
    useEffect(() => {
      if (queryResolveMetadataAsModel.data) {
        const metadataAsModel = cloneDeep(
          queryResolveMetadataAsModel.data.resolveMetadataAsModel
        );
    ...
    useEffect(() => {
      if (
        !isEmpty(queryResolveMetadataAsModel.error) ||
        !isEmpty(chosenMetadataError)
      ) {
        if (reloadFormMetadata) {
          reloadFormMetadata(
            {
            ...
            },
            {
              ...
              error:
                chosenMetadataError ??
                (queryResolveMetadataAsModel.error as unknown),
            } as MetadataFile
          );
        }
      }
      ...
      if (queryValidateSource.data) {
        const directory = selection.files.length
          ? selection.folderChain
              ...
              .join('/')
          : '';
        const fileNames = selection.files
          .map((file: FileData) => file.name)
          .join(',');
        if (
          queryValidateSource.data?.validateSource[ONLY_ONE_SOURCE].isValid ===
          false
        ) {
          if (reloadFormMetadata) {
            reloadFormMetadata(
                    ...
                    errors: [
                      {
                        message: intl.formatMessage(
                          ...
                          {
                            value:
                              queryValidateSource.data.validateSource[
                                ONLY_ONE_SOURCE
                              ].message,
                          }
                        ),
    ...
    const checkIsValidMetadata = useCallback(
