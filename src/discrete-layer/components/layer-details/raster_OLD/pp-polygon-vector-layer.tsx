...
export const PolygonPartsByPolygonVectorLayer: React.FC<PolygonPartsVectorLayerProps> =
  observer(
    ({ layerRecord, maskFeature, partsToCheck, ingestionResolutionMeter }) => {
      const store = useStore();
      const intl = useIntl();
      const mapOl = useMap();
      const [existingPolygonParts, setExistingPolygonParts] = useState<
        Feature[]
      >([]);
        ...
        setQuery(
          store.queryGetPolygonPartsFeature({
            data: {
              ...
              typeName: getWFSFeatureTypeName(
      ...
      useEffect(() => {
        if (maskFeature) {
          ...
          getExistingPolygoParts(
            convertFeatureToPolygon(maskFeature),
            START_PAGE
          );
        }
      }, [maskFeature]);
      ...
      useEffect(() => {
        if (!loading && data && maskFeature) {
          setExistingPolygonParts([
            ...(page === 0
              ? existingPolygonParts.slice(1)
              : existingPolygonParts),
            ...(data.getPolygonPartsFeature.features as Feature<
              Geometry,
              GeoJsonProperties
            >[]),
          ]);
      ...
      useEffect(() => {
        const interPartsSet = new SetWithContentEquality<Feature>(
          (part) => part.properties?.key
        );
        if (
          doneFetchingPP &&
          ...
          const check = async () => {
            ...
            for (let idx = 0; idx < totalParts; idx++) {
              ...
              let bufferedPart = buffer(
                part as Feature<Polygon>,
                EXISTING_PART_BUFFER_METERS_TOLLERANCE,
                { units: 'meters' }
              );
              ...
              if (
                !bufferedPart &&
                area(part as Feature<Polygon>) > MINIMAL_POLYGON_AREA_METERS
              ) {
                ...
                try {
                  if (bufferedPart) {
                    ...
                    if (
                      intersection &&
                      ingestionResolutionMeter >
                        existingPart.properties?.resolutionMeter
                    ) {
                  ...
                  dispatchAction({
                    ...
                    data: {
                      error: [
                        intl.formatMessage({
                          id: 'validation-general.polygonParts.wfsExtremelySmall',
                        }),
                      ],
                    },
                  });
