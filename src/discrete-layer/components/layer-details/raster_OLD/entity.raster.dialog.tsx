...
const setDefaultValues = (
  ...
  getFlatEntityDescriptors(record['__typename'] as LayerRecordTypes, descriptors).forEach(
    (field) => {
...
export const EntityRasterDialog: React.FC<EntityRasterDialogProps> = observer(
  (props: EntityRasterDialogProps) => {
    ...
    const [vestValidationResults, setVestValidationResults] = useState<Record<string, DraftResult>>(
    ...
    const checkHasQueriesSucceeded = (): boolean => {
      ...
      const hasAnyQuerySucceeded = Object.entries(mutationQuery.data ?? {}).some(
        ([key, val]) => mutationServices.includes(key) && val === SUCCESS_RESPONSE_VAL
      );
    ...
    useLayoutEffect(() => {
      ...
      if (dialogContainerRef.current !== null) {
        const baseContentHeight = getComputedStyle(dialogContainerRef.current).getPropertyValue(
          '--base-content-height'
        );
    ...
    const schemaUpdater = (partsNumber: number, startIndex = 0, removePrevNested = false) => {
      ...
      const newSchema = {
        ...(removePrevNested ? removePropertiesWithPrefix(schema, NESTED_FORMS_PRFIX) : schema),
      };

      for (let i = 0; i < partsNumber; i++) {
        newSchema[`${NESTED_FORMS_PRFIX}${i + startIndex}`] = Yup.object().shape({
          ...nestedYupSchema,
        });
      }
      setSchema(newSchema);
    ...
    useEffect(() => {
      ...
      [...ingestionFields, ...uiIngestionFieldDescriptors, ...descriptors].forEach((field) => {
        const fieldName: string = field.fieldName as string;
        switch (mode) {
          case Mode.NEW:
          case Mode.UPDATE:
    ...
    return (
      <div id="entityRasterDialog" ref={dialogContainerRef}>
        <Dialog open={isOpen} preventOutsideDismiss={true}>
          <DialogTitle
            style={
              mode !== Mode.NEW ? getTextStyle(layerRecord as any, 'backgroundColor') : undefined
            }
          >
            {dialogTitle}
            <IconButton
            ...
            {isAllInfoReady && (
              <EntityRasterForm
                ...
                onSubmit={(values): void => {
                  ...
                  // eslint-disable-next-line
                  const vestSuiteTopLevel = suite(descriptors as FieldConfigModelType[], values);
