...
export const InnerRasterForm = (
  ...
  const [clientCustomValidationErrors, setClientCustomValidationErrors] = useState<
  ...
  useEffect(() => {
    setShowCurtain(!isSelectedFiles || (isSelectedFiles && gpkgValidationError !== undefined));
  ...
  useEffect(() => {
    ...
    if (countPPWithCertainGeometryErrors > NONE) {
      const footprintErrorsRatio =
        ...
        Object.keys(values).filter((key) => key.includes(NESTED_FORMS_PRFIX)).length;
        ...
        Object.keys(errObj).forEach((fieldKey) => {
          ...
          errObj[fieldKey] = errObj[fieldKey].codes.map((errText) =>
            intl.formatMessage(
              { id: errText },
              {
                fieldName: emphasizeByHTML(`${intl.formatMessage({ id: errObj[fieldKey].label })}`),
              }
            )
          );
        });
    ...
    return getEnumKeys(enumsMap, 'ProviderType').map((key) => {
      ...
      return {
        label: intl.formatMessage({
          id: `enum-value.provider_type.${ProviderType[value].toLowerCase()}.label`,
        }),
        ...
        categories: desc.categories?.map((cat) => {
          return {
            ...
            fields: cat.fields.filter(
              (field: FieldConfigModelType) => !field.isAutoGenerated && !field.isLifecycleEnvolved
            ),
          };
        }),
      };
          ...
          <Handler
            ...
            handleClick={() => {
              ...
              if (status) {
                const { errors } = status;
                if (errors) {
                  const { [currentFormKey]: removedKey, ...rest } = errors as Record<
                    string,
                    unknown
                  >;
      ...
      >
        ...
        <Box className="footer">
          <Box className="messages">
            {isFirstPhaseErrorInPolygonPart && (
              <ValidationsError
                errors={firstPhaseErrors[currentFormKey] as unknown as Record<string, string[]>}
              />
            )}
                    ...
                    onClick={(): void => {
                      ...
                      const polygonData = {
                        ...
                        sourceResolutionMeter: Object.values(ZOOM_LEVELS).find(
                          (zoom) => zoom.resolutionDeg === gpkgValidationResults?.resolutionDegree
                        )?.resolutionMeter,
