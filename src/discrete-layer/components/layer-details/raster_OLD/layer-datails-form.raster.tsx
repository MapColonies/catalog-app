...
import {
  getGraphQLPayloadNestedObjectErrors,
  getUIIngestionFieldDescriptors,
} from './utils';
...
export const InnerRasterForm = (
  ...
  const [loadingPolygonParts, setLoadingPolygonParts] =
    useState<boolean>(false);
  const [outlinedPerimeter, setOutlinedPerimeter] = useState<
    Feature | undefined
  >();
  const [outlinedPerimeterMarker, setOutlinedPerimeterMarker] = useState<
    Feature | undefined
  >();
  const [sourceExtent, setSourceExtent] = useState<Feature | undefined>();
  const [sourceExtentMarker, setSourceExtentMarker] = useState<
    Feature | undefined
  >();
  const [selectedFeature, setSelectedFeature] = useState<string | undefined>();
  const [expandedParts, setExpandedParts] = useState<boolean[]>([]);
  const [showPolygonPartsOnMap, setShowPolygonPartsOnMap] =
    useState<boolean>(true);
  const [showFaultyPolygonParts, setShowFaultyPolygonParts] =
    useState<boolean>(false);
  const [faultyPolygonParts, setFaultyPolygonParts] = useState<string[]>([]);
  const [showExisitngLayerPartsOnMap, setShowExisitngLayerPartsOnMap] =
    useState<boolean>(false);
  const [polygonPartsMode, setPolygonPartsMode] =
    useState<POLYGON_PARTS_MODE>('MANUAL');
  const [layerPolygonParts, setLayerPolygonParts] = useState<
    Record<string, PolygonPartRecordModelType>
  >({});
  const [ppCheckPerformed, setPPCheckPerformed] = useState<boolean>(false);
  const [gpkgValidationError, setGpkgValidationError] = useState<
    string | undefined
  >(undefined);
  const [clientCustomValidationErrors, setClientCustomValidationErrors] =
    useState<Record<string, string>>({});
  const [gpkgValidationResults, setGpkgValidationResults] = useState<
    SourceValidationModelType | undefined
  >(undefined);
  const [graphQLPayloadObjectErrors, setGraphQLPayloadObjectErrors] = useState<
    number[]
  >([]);
  const [isSubmittedForm, setIsSubmittedForm] = useState(false);
  const [isThresholdErrorsCleaned, setIsThresholdErrorsCleaned] =
    useState(false);
  ...
  useEffect(() => {
    setShowCurtain(
      !isSelectedFiles || (isSelectedFiles && gpkgValidationError !== undefined)
    );
  }, [isSelectedFiles, gpkgValidationError]);

  useEffect(() => {
    setGraphQLPayloadObjectErrors(
      getGraphQLPayloadNestedObjectErrors(mutationQueryError)
    );
  ...
  useEffect(() => {
    ...
    Object.keys(values)
      ...
      .forEach((key) => {
        ...
        // @ts-ignore
        countPPWithGeometryErrors += !isGeometryPolygon(values[key].footprint)
          ? 1
          : 0;
      });
    ...
    if (countPPWithGeometryErrors > NONE) {
      setClientCustomValidationErrors({
        ...
        [CUSTOM_VALIDATION_ERROR_CODES.POLYGON_PARTS_NOT_VALID_GEOMETRY]:
          intl.formatMessage(
  ...
  useEffect(() => {
    ...
    const getCountPPWithGeometryErrors = (
      features: Record<string, unknown>,
      errorsToCheck: Record<string, string> | undefined = undefined
    ): number => {
    ...
    const addFootPrintAmountClientError = (count: number) => {
      setClientCustomValidationErrors({
        ...clientCustomValidationErrors,
        // eslint-disable-next-line @typescript-eslint/no-use-before-define
        [CUSTOM_VALIDATION_ERROR_CODES.POLYGON_PARTS_NOT_VALID_FOOTPRINT]:
          intl.formatMessage(
    ...
    const cleanUpFieldErrorMsg = (
      ...
      if (!isEmpty(fieldErrors)) {
        const err_idx = fieldErrors.findIndex(
          (elem: string) => elem === intl.formatMessage({ id: err })
        );
        if (err_idx > -1) {
    ...
    let effectiveStatusErrors = {
      ...(status?.errors as { [key: string]: any }),
    };

    if (effectiveStatusErrors) {
      countPPWithCertainGeometryErrors = getCountPPWithGeometryErrors(
        effectiveStatusErrors,
        GEOMETRY_ERRORS_THRESHOLD
      );
      countPPWithAllGeometryErrors = getCountPPWithGeometryErrors(
        effectiveStatusErrors
      );
    }
    ...
    if (countPPWithCertainGeometryErrors > NONE) {
      const footprintErrorsRatio =
        countPPWithCertainGeometryErrors /
        Object.keys(values).filter((key) => key.includes(NESTED_FORMS_PRFIX))
          .length;
      if (
        footprintErrorsRatio <= CONFIG.POLYGON_PARTS.GEOMETRY_ERRORS_THRESHOLD
      ) {
        Object.values(GEOMETRY_ERRORS_THRESHOLD).forEach((err) => {
          if (effectiveStatusErrors) {
            Object.keys(effectiveStatusErrors)
              .filter((key) => key.includes(NESTED_FORMS_PRFIX))
              .forEach((key) => {
                cleanUpFieldErrorMsg(
                  get(effectiveStatusErrors, `${key}`),
                  'footprint',
                  err
                );
        ...
        const effectiveStatusErrorsLength = Object.keys(
          effectiveStatusErrors
        ).length;
        ...
        if (effectiveStatusErrorsLength > 0) {
          effectiveStatusErrors[POLYGON_PARTS_STATUS_ERROR] = [
            intl.formatMessage(
              { id: 'validation-general.polygonParts.hasErrors' },
              {
                numErrorParts: emphasizeByHTML(
                  `${effectiveStatusErrorsLength}`
                ),
              }
            ),
          ];
        ...
        const errorsCount =
          countPPWithAllGeometryErrors - countPPWithCertainGeometryErrors;
  ...
  useEffect(() => {
    if (polygonPartsMode === 'MANUAL' && ppFeatures.length) {
      const definedPPGeometries = ppFeatures.filter(
        (item) => !isEmpty(item.geometry)
      );
      if (definedPPGeometries.length) {
        ...
        setOutlinedPerimeter(
          outlinedPolygon as Feature<Geometry, GeoJsonProperties>
        );
  ...
  useEffect(() => {
    const stattusErrors = parsingErrors.reduce((acc, curr) => {
      Object.keys(curr).forEach((key) => {
        let errObj = curr[key] as Record<string, ParsedPolygonPartError>;
        Object.keys(errObj).forEach((fieldKey) => {
          // @ts-ignore
          errObj[fieldKey] = errObj[fieldKey].codes.map((errText) =>
            intl.formatMessage(
              { id: errText },
              {
                fieldName: emphasizeByHTML(
                  `${intl.formatMessage({ id: errObj[fieldKey].label })}`
                ),
              }
            )
          );
  ...
  useEffect(() => {
    const vestValidationPPKeys = Object.fromEntries(
      Object.entries(vestValidationResults).filter(
        ([key, value]) => vestValidationResults[key].errorCount
      )
    );
    const graphQLPPKeys = Object.fromEntries(
      graphQLPayloadObjectErrors.map((k) => [NESTED_FORMS_PRFIX + k, k])
    );
    const ppWithProblems = Object.keys({
      ...firstPhaseErrors,
      ...vestValidationPPKeys,
      ...graphQLPPKeys,
    }).filter((key) => key.includes(NESTED_FORMS_PRFIX));
    ...
    return getEnumKeys(enumsMap, 'ProviderType').map((key) => {
      ...
      return {
        label: intl.formatMessage({
          id: `enum-value.provider_type.${ProviderType[
            value
          ].toLowerCase()}.label`,
        }),
  ...
  const exceededFeaturesNumberError = useMemo(
    () =>
      new Error(
        intl.formatMessage(
  ...
  const exceededVertexNumberError = useCallback(
    (numberOfPP: number, numberOfVertexes: number) => {
      return new Error(
        intl.formatMessage(
  ...
  const shapeFileGenericError = useMemo(
    () => new Error(`validation-general.shapeFile.generic`),
    []
  );
          ...
          return {
            ...
            fields: cat.fields.filter(
              (field: FieldConfigModelType) =>
                !field.isAutoGenerated && !field.isLifecycleEnvolved
            ),
          };
        }),
      };
  ...
  const reloadFormMetadata = (
    ...
    const validationResults =
      metadata.recordModel as unknown as SourceValidationModelType;
    ...
    setGpkgValidationError(
      !validationResults.isValid
        ? (validationResults.message as string)
        : undefined
    );
    ...
    const relevantValues = removePrevData
  ...
  const isShapeFileValid = (
    featuresArr: Feature<Geometry, GeoJsonProperties>[]
  ): boolean | Error => {
    let verticesNum = 0;
  ...
  const transformShapeFeatureToEntity = (
    polygonPartDescriptors: FieldConfigModelType[],
    ...
    switch (provider) {
      case ProviderType.SYNERGY:
        ret = transformSynergyShapeFeatureToEntity(
          polygonPartDescriptors,
          feature,
          provider,
          fileName
        );
      ...
      case ProviderType.TERRA_NOVA:
        ret = transformTeraNovaShapeFeatureToEntity(
          polygonPartDescriptors,
          feature,
          provider,
          fileName
      ...
      case ProviderType.MAXAR:
        ret = transformMaxarShapeFeatureToEntity(
          polygonPartDescriptors,
          feature,
          provider,
          fileName
      ...
      try {
        ...
        if (fileType === ZIP_EXTENSION) {
          void shp(shapeArrayBuffer)
            .then((data) => {
              const polygonPartDescriptors = getFlatEntityDescriptors(
                'PolygonPartRecord',
                entityDescriptors
              );
              const parsedPolygonParts: Record<string, ParsedPolygonPart> = {};

              const shapeFileValidation = isShapeFileValid(
                (data as FeatureCollectionWithFilename).features
              );
              if (shapeFileValidation instanceof Error) {
                return reject(shapeFileValidation);
              }

              (data as FeatureCollectionWithFilename).features.forEach(
                (feature, idx) => {
                  const currentKey = `${NESTED_FORMS_PRFIX}${idx}`;
                  const parsedPolygonPartData = transformShapeFeatureToEntity(
                    polygonPartDescriptors,
                    feature,
                    provider,
                    fileName
              ...
              const outlinedPolygon = getOutlinedFeature(
                (data as FeatureCollectionWithFilename).features as Feature<
              ...
              setOutlinedPerimeterMarker({
                ...
                geometry: {
                  coordinates: getFirstPoint(
                    (outlinedPolygon as Feature).geometry
                  ),
                  type: 'Point',
                },
              });
  ...
  firstPhaseErrors &&
    Object.keys(firstPhaseErrors).forEach((err) => {
  ...
  const Handler: React.FC<HandleProps> = ({
    partIndex,
    text,
    onClick,
  ...
  const geoArgsParams: geoArgs = [
    { name: NATIVE_RESOLUTION_NAME_FIELD, value: undefined },
  ];
  const getResolutionObject = (params: geoArgs) =>
    params.find((param) => param.name === NATIVE_RESOLUTION_NAME_FIELD);
  ...
  const ppVertexDensityFactor = (geometry: any, params: geoArgs) => {
    ...
    const densityFactor = polygonVertexDensityFactor(
      geometry,
      resolutionObj.value * DEGREES_PER_METER
    );
  ...
  const customChecks = useMemo(() => {
    return {
      validationFunc: [ppVertexDensityFactor, ppArea, ppCountSmallHoles],
  ...
  const renderRow: ListRowRenderer = ({ index, key, style }) => {
      ...
      for (let i = 0; i < Object.keys(dataKeys).length; i++) {
        for (let j = 0; j < Object.keys(faultyPolygonParts).length; j++) {
          if (dataKeys[i] === faultyPolygonParts[j]) {
            data.push(
              Object.values(layerPolygonParts)[
                i
              ] as unknown as PolygonPartRecordModelType
            );
          }
        }
      }
    ...
    const isFirstPhaseErrorInPolygonPart =
      firstPhaseErrors[currentFormKey] &&
      Object.keys(firstPhaseErrors[currentFormKey])?.length > NONE;
    const isVestPhaseErrorInPolygonPart =
      vestValidationResults[currentFormKey] &&
      vestValidationResults[currentFormKey]?.errorCount > NONE;
    const isGraphQLErrorInPolygonPart =
      graphQLPayloadObjectErrors.includes(index);
        ...
        handle={
          <Handler
            ...
            isErrorInPolygonPart={
              ...
              isVestPhaseErrorInPolygonPart ||
              isGraphQLErrorInPolygonPart ||
              !isGeometryValid
            }
            handleClick={() => {
              ...
              if (status) {
                const { errors } = status;
                if (errors) {
                  const { [currentFormKey]: removedKey, ...rest } =
        ...
        <Box className="footer">
          <Box className="messages">
            {isFirstPhaseErrorInPolygonPart && (
              <ValidationsError
                errors={
                  firstPhaseErrors[currentFormKey] as unknown as Record<
                    string,
                    string[]
                  >
                }
            ...
            {!isFirstPhaseErrorInPolygonPart &&
              isVestPhaseErrorInPolygonPart && (
                <ValidationsError
                  errors={vestValidationResults[currentFormKey].getErrors()}
                />
              )}
          </Box>
        </Box>
          ...
          >
            <Select
              ...
              placeholder={intl.formatMessage({
                id: `polygon-parts.button.load-from-shapeFile`,
              })}
              options={shapeFileProviders}
              disabled={
                /*!isIngestedSourceSelected() &&*/ showCurtain ||
                isValidatingSource
              }
              onClick={(e): void => {
                  ...
                  (ev, fileType, fileName) => {
                    ...
                    void proccessShapeFile(
                      ...
                      .then((parsedPPData) => {
                        ...
                        setExpandedParts(
                          new Array(ppDataKeys.length).fill(false)
                        );

                        const polygonsData = ppDataKeys
                          .map((key) => {
                            // eslint-disable-next-line @typescript-eslint/no-unused-vars
                      ...
                      .catch((e) => {
                        ...
                        setStatus({
                          errors: {
                            // ...currentErrors,
                            [POLYGON_PARTS_STATUS_ERROR]: [
                              intl.formatMessage({ id: (e as Error).message }),
                            ],
                          },
                        });
                      });
          ...
          <Box className="checkBoxContainer displayFlex">
            <Box className="polygonPartsData showOnMapContainer displayFlex">
              <Checkbox
                ...
                label={intl.formatMessage({
                  id: 'polygon-parts.show-parts-on-map.label',
                })}
              ...
              <Checkbox
                ...
                label={intl.formatMessage({
                  id: 'polygon-parts.show-parts-with-errors-on-map.label',
                })}
            ...
            <Box className="displayFlex">
              {mode === Mode.UPDATE && (
                <Checkbox
                  ...
                  label={intl.formatMessage({
                    id: 'polygon-parts.show-exisitng-parts-on-map.label',
                  })}
            ...
            <Box className="polygonPartsData">
              {!loadingPolygonParts && isEmpty(layerPolygonParts) && (
                <Typography
                  ...
                  style={{
                    paddingTop: '120px',
                    height: polygonPartsMode === 'MANUAL' ? '370px' : '410px',
                  }}
                >
              ...
              {!loadingPolygonParts && !isEmpty(layerPolygonParts) && (
                <List
                  ...
                  rowCount={
                    faultyPolygonParts.length && showFaultyPolygonParts
                      ? faultyPolygonParts.length
                      : expandedParts.length
                  }
                  overscanRowCount={3}
                  rowHeight={(idx) => (expandedParts[idx.index] ? 316 : 48)}
                  scrollToIndex={
                    graphQLPayloadObjectErrors[0]
                      ? graphQLPayloadObjectErrors[0]
                      : expandedParts.length
                      ? expandedParts.length - 1
                      : 0
                  }
                />
              )}
              {!loadingPolygonParts && polygonPartsMode === 'MANUAL' && (
                <Box className="addPolygonPart">
                  <Button
                    outlined
                    type="button"
                    ...
                    onClick={(): void => {
                      ...
                      const exisitngParts = layerPolygonParts
                        ? Object.keys(layerPolygonParts)
                            .map((key) =>
                              parseInt(key.replace(NESTED_FORMS_PRFIX, ''))
                            )
                            .sort((a, b) => b - a)
                        : [];
                      const lastPartIdx = exisitngParts.length
                        ? exisitngParts[0] + 1
                        : 0;
                      ...
                      const polygonData = {
                        ...
                        sourceResolutionMeter: Object.values(ZOOM_LEVELS).find(
                          (zoom) =>
                            zoom.resolutionDeg ===
                            gpkgValidationResults?.resolutionDegree
                        )?.resolutionMeter,
                  ...
                  geoFeatures={
                    showPolygonPartsOnMap
                      ? [
                          ...
                          sourceExtentMarker as Feature<
                            Geometry,
                            GeoJsonProperties
                          >,
                          outlinedPerimeter as Feature<
                            Geometry,
                            GeoJsonProperties
                          >,
                          outlinedPerimeterMarker as Feature<
                            Geometry,
                            GeoJsonProperties
                          >,
                          ...ppFeatures,
        ...
        <Box className="footer">
          <Box className="messages">
            ...
            {(Object.keys(topLevelFieldsErrors).length === NONE ||
              ...
              vestValidationResults.topLevelEntityVestErrors?.errorCount >
                NONE && (
                <ValidationsError
                  errors={vestValidationResults.topLevelEntityVestErrors.getErrors()}
                />
              )}
          ...
          <Box className="buttons">
            {mode === Mode.NEW ||
            (ppCheckPerformed &&
              ppCollisionCheckInProgress === false &&
