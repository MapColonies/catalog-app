...
export const getEntityDescriptors = (
  ...
  switch (layerRecordTypename) {
    ...
    case 'QuantizedMeshBestRecord':
      entityDesc = entityDescriptors.find(
        (descriptor) => descriptor.type === 'PycswQuantizedMeshBestCatalogRecord'
      );
      break;
...
export const getFieldNamesByEntityDescriptorMap = (
  ...
  LayerRecordTypesKeys.forEach((layerRecordTypename: string) => {
    const fieldNames = extractDescriptorRelatedFieldNames(
      descriptor,
      getFlatEntityDescriptors(layerRecordTypename as LayerRecordTypes, entityDescriptors)
    );
...
export const removeEmptyObjFields = (obj: Record<string, unknown>): Record<string, unknown> => {
...
export const getYupFieldConfig = (field: FieldConfigModelType, intl: IntlShape): MixedSchema => {
    ...
    if (shapeFieldValue) {
      switch (desc.fieldName) {
        ...
        case 'imagingTimeEndUTC':
          poygonPartData[desc.fieldName as string] = moment(shapeFieldValue, 'DD/MM/YYYY').add(
            HOURS_TO_ADD,
            'hours'
          );
  ...
  desciptors.forEach((desc) => {
    ...
    if (shapeFieldValue) {
      switch (desc.fieldName) {
        case 'imagingTimeBeginUTC':
        case 'imagingTimeEndUTC':
          poygonPartData[desc.fieldName as string] = moment(shapeFieldValue, 'YYYY-MM-DD').add(
            HOURS_TO_ADD,
            'hours'
          );
          if ((poygonPartData[desc.fieldName as string] as moment.Moment) > moment()) {
...
export const getRecordForUpdate = (
  ...
  const handleExplicitOperation = (
    ...
    if (fieldOperation.value === null || typeof fieldOperation.value === 'undefined')
      return recordCopy;
    ...
    if (fraction in versionFractions) {
      const currentVersionSplit = (recordCopy[field.fieldName as string] as string).split(
        VERSION_DELIMITER
      );
  ...
  for (const field of updateRulesFields) {
    ...
    const fieldOperation = get(updateRules, 'value.operation') as
      | UpdateRulesOperationModelType
      | undefined;
    const selectedLayerFieldValue = (selectedLayer as unknown as Record<string, unknown>)[
