...
export const LookupOptionsPresentorComponent: React.FC<LookupTablesPresentorProps> = (props) => {
  ...
  const customStyles = {
    container: (styles: any, { data, isDisabled, isFocused, isSelected }: any) => {
      return {
        marginRight:
          mode === Mode.EDIT
            ? '18px'
            : (layerRecord as LayerMetadataMixedUnion)?.type === RecordType.RECORD_3D
            ? '16px'
            : '12px',
        marginLeft: '2px',
        width: '100%',
    ...
    control: (styles: any, { data, isDisabled, isFocused, isSelected }: any) => {
      return {
        ...
        borderColor: isFocused
          ...
          : CustomTheme.darkTheme.GC_TAB_ACTIVE_BACKGROUND,
        borderRadius: '4px',
        display: 'flex',
        alignItems: 'center',
    ...
    option: (styles: any, { data, isDisabled, isFocused, isSelected }: any) => {
      return {
        ...
        height: '40px',
        color: isSelected
          ? CustomTheme.darkTheme.GC_HOVER_BACKGROUND
    ...
    indicatorsContainer: (styles: any, { data, isDisabled, isFocused, isSelected }: any) => {
      return {
        ...
        padding:
          mode === Mode.NEW &&
          (layerRecord as LayerMetadataMixedUnion)?.type === RecordType.RECORD_RASTER
            ? '0 4px'
            : '0 22px',
      };
    },
    ...
    if (isEmpty(innerValue)) {
      ...
      const displayValue = Object.keys(lookupOptions).includes('translation')
        ? filteredOptions.length
          ? (filteredOptions[0] as ILookupOption).translation?.find((trns) => trns.locale === lang)
              ?.text
          : value
        : filteredOptions.length
  ...
  const lookupOptions = !fieldInfo.isMultiSelection
    ? [
  ...
  return fieldInfo.isMultiSelection ? (
    <MultiSelection
      fieldInfo={fieldInfo}
