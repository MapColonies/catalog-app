...
export const ResolutionValuePresentorComponent: React.FC<ResolutionValuePresentorProps> = (
  props
) => {
  ...
  useEffect(() => {
    if (lookupOptions) {
      const filteredOptions = lookupOptions?.filter(
        (option) =>
          option.properties[fieldInfo.lookupTableBinding.valueFromPropertyName] === Number(value)
      );
      if (!!filteredOptions?.length) {
        formik?.setFieldValue(
          `${fieldNamePrefix ?? ''}${fieldInfo.dependentField.name}` as string,
          filteredOptions[0].properties[fieldInfo.dependentField.valueFromPropertyName]
        );
      }
    }
  }, [value]);

  if (!lookupTablesData || !lookupTablesData.dictionary || fieldInfo.lookupTable == null)
    return null;
  ...
  if (
    ...
    return (
      <TooltippedValue
        className={`detailsFieldValue  ${isDataError ? 'detailFieldDataError' : ''}`}
      >
        {getDisplayValue()}
      </TooltippedValue>
    );
    ...
    <Box className="detailsFieldValue selectBoxContainer resolutionBoxContainer">
      <Select
        ...
        onChange={(e: React.FormEvent<HTMLSelectElement>): void => {
          ...
          const filteredOptions = lookupOptions.filter(
            (option) =>
              option.properties[fieldInfo.lookupTableBinding.valueFromPropertyName] ===
        ...
        {lookupOptions
          ...
          .map(({ translationCode, value, properties }, index) => {
            ...
            return (
              <MenuItem
                key={index}
                value={properties[fieldInfo.lookupTableBinding.valueFromPropertyName] as number}
              >
