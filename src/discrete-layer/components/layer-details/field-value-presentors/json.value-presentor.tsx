...
export const JsonValuePresentorComponent: React.FC<JsonValuePresentorProps> = ({
  ...
  const multipleFeaturesError = useMemo(
    () =>
      new Error(`validation-field.${fieldInfo.fieldName as string}.shapeFile.multiple-features`),
    [fieldInfo.fieldName]
  );
  const shapeFileGenericError = useMemo(
    () => new Error(`validation-field.${fieldInfo.fieldName as string}.shapeFile.generic`),
    [fieldInfo.fieldName]
  );
  ...
  const debouncedRemoveStatusErrors = useDebouncedCallback(removeStatusErrors, REMOVE_ERROR_DELAY);
      ...
      try {
        ...
        if (isFieldRequired || jsonValue.length > NONE) {
          ...
          set(
            errs,
            fieldName,
            Array.from(new Set([...(get(currentErrors, fieldName) ?? []), errorMsg]))
          );
          setTimeout(() => {
            formik?.setStatus({
              errors: errs,
            ...
            <Box className="jsonDetailsFieldTextValueContainer">
              ...
              {geoJsonWarning !== '' && (
                <Box className="warningContainer">
                  <Typography tag="span" dangerouslySetInnerHTML={{ __html: geoJsonWarning }} />
                </Box>
              )}
            </Box>
          </Box>
