...
import { Button, Icon, Switch, Typography } from '@map-colonies/react-core';
...
const FileItem: React.FC<{ file: any; context: IContext }> = ({ file, context }) => {
  const color =
    !file.isExists || file.hasError ? 'error' : file.isModDateDiffExceeded ? 'warning' : '';
  ...
  return (
    <>
      ...
      <Box className={`ltr ${color}`}>{defaultFormatters.formatFileSize(null, file.details)}</Box>
      ...
      if (selected.files.length) {
        ...
        if (selectedAction) {
          const actionTypeMap: Record<
            string,
            {
              type: 'SELECT_FILES' | 'SELECT_DATA' | 'SELECT_PRODUCT' | 'SELECT_SHAPEMETADATA';
      ...
      <>
        ...
        <Box className={`header section ${curtain ? 'curtainContainer' : ''}`}>
          <Box className="ingestionFields">
            {isLoading ? <Skeleton width="99%" count={3} /> : <IngestionInputs state={state} />}
            {curtain && <Curtain showProgress={false} />}
          </Box>
          {hasActiveJob(state.context) === false && selectionMode === MANUAL && (
