...
import { SelectionMode, Events, IFiles, IContext } from './state-machine/types';
...
const FileItem: React.FC<{ file: any; context: IContext }> = ({
  file,
  context,
}) => {
  const color =
    !file.isExists || file.hasError
      ? 'error'
      : file.isModDateDiffExceeded
      ? 'warning'
      : '';
  ...
  return (
    <>
      ...
      <Box className={`fileItemName ${color}`}>{formatPath(file.path)}</Box>
    ...
    <Box className="ingestionField">
      ...
      <Box className="detailsFieldValue">
        ...
        <Box className="filesList">
          {state.context?.files &&
            Object.values(state.context?.files as IFiles).map(
              (file: any, idx: number): JSX.Element => {
                return (
                  <FileItem key={idx} file={file} context={state.context} />
                );
              }
            )}
        </Box>
...
export const IngestionFields: React.FC<IngestionFieldsProps> = observer(
  (props: IngestionFieldsProps) => {
    ...
    const [selectionMode, setSelectionMode] = useState<SelectionMode>(
      state.context.selectionMode as SelectionMode
    );
    ...
    const [pendingFileEvent, setPendingFileEvent] = useState<Events | null>(
      null
    );
    const [IngestionFilesTypeConfig, setIngestionFilesTypeConfig] =
      useState<RasterIngestionFilesTypeConfig>();
        ...
        if (selectedAction) {
          const actionTypeMap: Record<
            string,
            {
              ...
              predicate: (modeDate: Date | string) => string;
            }
          > = {
            ...
            product: {
              type: 'SELECT_PRODUCT',
              predicate: relativeDateFormatter,
            },
            shapeMetadata: {
              type: 'SELECT_SHAPEMETADATA',
              predicate: relativeDateFormatter,
            },
          };
    ...
    const isFileSelectionDisabled = (fileName: keyof IFiles) => {
      return (
        ...
        state.context.files?.[fileName]?.isDisabled
      );
    };
      ...
      <>
        {CONFIG.SHOW_SELECTION_MODE_SWITCH &&
          ...
          (selectionMode === AUTO || selectionMode === MANUAL) && (
            <Box
              className={`ingestionSwitchContainer ${
                state.context.flowType === Mode.UPDATE ? 'update' : ''
              } ${isUIDisabled(isLoading, state) ? 'disabled' : ''}`}
            >
        ...
        <Box className={`header section ${curtain ? 'curtainContainer' : ''}`}>
              ...
              <Box className="ingestionManualButtons">
                <Button
                  ...
                  onClick={() => {
                    ...
                    setIngestionFilesTypeConfig(
                      RasterIngestionFilesTypeConfig.DATA
                    );
                  }}
                >
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    setFilePickerDialogOpen(true);
                    setIngestionFilesTypeConfig(
                      RasterIngestionFilesTypeConfig.PRODUCT
                    );
                  }}
                >
                  <FormattedMessage id="general.choose-btn.text" />
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    setFilePickerDialogOpen(true);
                    setIngestionFilesTypeConfig(
                      RasterIngestionFilesTypeConfig.SHAPEMETADATA
                    );
                  }}
                >
                  <FormattedMessage id="general.choose-btn.text" />
          ...
          {hasActiveJob(state.context) === false && selectionMode === AUTO && (
