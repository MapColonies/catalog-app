          ...
          {existingPolygonParts.map((feat, idx) => {
            const greenStyle = new Style({
              text: createTextStyle(
                ...
                ZOOM_LEVELS_TABLE,
                intl.formatMessage({
                  id: 'polygon-parts.map-preview.zoom-before-fetch',
                })
              ),
            ...
            const bbox = bboxPolygon(
              mapOl
                .getView()
                .calculateExtent([size[0] + BUFFER, size[1] + BUFFER]) as BBox
            );
            ...
            try {
              ...
              if (featureClippedPolygon) {
                const geometry = new GeoJSON().readGeometry(
                  featureClippedPolygon.geometry
                );
