...
import { FeatureCollection, Feature, Geometry, GeoJsonProperties, Polygon, MultiPolygon } from 'geojson';
...
const _downloadFile = async (url: string, onProgress?: (p: WorkerMessage | null) => void): Promise<ArrayBuffer|null> => {
  try {
...
const api: WorkerAPI = {
  ...
  async loadFromShapeFile(url: string, onProgress?: (p: WorkerMessage | null) => void): Promise<void> {
    api.dispose();
  ...
  updateAreas(onProgress?: (p: WorkerMessage | null) => void): void {
      ...
      for (let i = 0; i < total; i++) {
        ...
        _properties[i].area = _geos.Module.getValue(areaPtr, 'double') * Math.pow(ONE_DEGREE_KM,2) * Math.cos(lat * (Math.PI / 180));
    ...
    getFeatureCollection(onProgress?: (p: WorkerMessage | null) => void): FeatureCollection {
      const t0 = performance.now();
      const featuresArray = cloneDeep(_featureTemplate.map((item, idx) => {
        return {
      ...
      console.log('getFeatureCollection took:', _featureTemplate.length, '    ', performance.now() - t0, 'ms');
