...
export const getDetails = async (
  ...
  if (files) {
    return {
      ...files.filter(
        (file: FileData) => file.name === path.basename(filePath)
      )[0],
    };
  }
  return undefined;
...
export const selectData = async (context: IContext) => {
  ...
  if (!gpkgValidation.isValid) {
    throw buildError(
      'ingestion.error.invalid-source-file',
      gpkgValidation.message as string
    );
  }
...
export const getJob = async (context: IContext): Promise<JobModelType> => {
  ...
  const resolvedJob = merge({}, result.job, {
    parameters: {
      metadata: {
        ...filterByKeys(
          resolvedMetadata.resolveMetadataAsModel,
...
export const getRestoreData = async (
  context: IContext
): Promise<IPartialContext> => {
  ...
  try {
    return {
      flowType: jobType2Mode[
        job.type || RasterIngestionJobType.NEW
      ...
      files: {
        ...
        product: {
          ...
          path: getPath(
            MOUNT_DIR,
            job.parameters.inputFiles.productShapefilePath
          ),
          isExists: false,
        ...
        shapeMetadata: {
          ...
          path: getPath(
            MOUNT_DIR,
            job.parameters.inputFiles.metadataShapefilePath
          ),
          isExists: false,
          dateFormatterPredicate: relativeDateFormatter,
        },
