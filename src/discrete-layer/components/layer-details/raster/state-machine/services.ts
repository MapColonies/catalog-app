...
export const SERVICES = {
  [WORKFLOW.ROOT]: {
    fetchActiveJobService: fromPromise(
      async ({ input }: FromPromiseArgs<IContext>) => {
    ...
    jobSubmissionService: fromPromise(
      async ({ input }: FromPromiseArgs<IContext>) => {
        const { store, files, resolutionDegree, formData } =
          input.context || {};
        ...
        const data = {
          ...
          callbackUrls: [
            `${CONFIG.SERVICE_PROTOCOL}${CONFIG.SERVICE_NAME}/callback/task`,
          ],
    ...
    restoreJobService: fromPromise(
      async ({ input }: FromPromiseArgs<IContext>) => {
        ...
        if (files) {
          if (files.data) {
            files.data.details = await getDetails(
              files.data.path,
              input.context
            );
            ...
            if (files.data.isExists === false) {
              errors = [
                ...errors,
                buildError('ingestion.error.missing', 'GPKG'),
              ];
            } else {
              const gpkgValidation = await validateGPKG(
                files.data.path,
                input.context
              );
          ...
          if (files.product) {
            files.product.details = await getDetails(
              files.product.path,
              input.context
            );
            ...
            if (files.product.isExists === false) {
              errors = [
                ...errors,
                buildError('ingestion.error.missing', PRODUCT_FILENAME),
              ];
            } else {
              const productFile = await fetchProduct(
                files.product,
                input.context
              );
          ...
          if (files.shapeMetadata) {
            files.shapeMetadata.details = await getDetails(
              files.shapeMetadata.path,
              input.context
            );
            ...
            if (files.shapeMetadata.isExists === false) {
              errors = [
                ...errors,
                buildError('ingestion.error.missing', SHAPEMETADATA_FILENAME),
              ];
            }
          }
          if (
  ...
  [WORKFLOW.FILES.ROOT]: {
    selectFilesService: fromPromise(
      async ({ input }: FromPromiseArgs<IContext>) => {
    ...
    selectDataService: fromPromise(
      async ({ input }: FromPromiseArgs<IContext>) => {
        return selectData(input.context);
      }
    ),
    fetchProductService: fromPromise(
      async ({ input }: FromPromiseArgs<IContext>) => {
    ...
    checkShapeMetadataService: fromPromise(
      async ({ input }: FromPromiseArgs<IContext>) => {
