...
export const validateShapeFiles = (files: IFiles): IStateError[] => {
  ...
  if (productDetails && shapeMetadataDetails) {
    ...
    if (
      ...
      CONFIG.RASTER_INGESTION.CHANGES_IN_SHAPE_FILES.TIME_DIFFERENCE_GRACE_MINUTES
    ) {
      return [
        buildError(
...
export const hasError = (errors: IStateError[]): boolean => {
  return (
    errors.length > 0 &&
    (errors.some((error) => error.level === 'error') || errors.some((error) => !error.level))
  );
};
...
export const isModified = (modDate: Date | string) => {
  return modDate
    ? moment().diff(moment(modDate), 'hours') <=
        CONFIG.RASTER_INGESTION.CHANGES_IN_SHAPE_FILES.TIME_MODIFIED_THRESHOLD_HOURS
    : false;
};
