    ...
    [WORKFLOW.FILES.AUTO.ROOT]: {
      ...
      states: {
        [WORKFLOW.FILES.AUTO.IDLE]: {
          entry: () =>
            console.log(
              `>>> Enter ${WORKFLOW.FILES.AUTO.ROOT.toUpperCase()}.${WORKFLOW.FILES.AUTO.IDLE}`
            ),
          on: {
            SELECT_FILES: {
              actions: [
    ...
    [WORKFLOW.FILES.MANUAL.ROOT]: {
      ...
      states: {
        [WORKFLOW.FILES.MANUAL.IDLE]: {
          ...
          on: {
            SELECT_DATA: {
              actions: [...selectFileActions('data'), ...cleanFilesErrorActions()],
              target: WORKFLOW.FILES.MANUAL.SELECT_DATA,
            },
            SELECT_PRODUCT: {
              actions: [...selectFileActions('product'), ...cleanFilesErrorActions()],
              target: WORKFLOW.FILES.MANUAL.FETCH_PRODUCT,
            },
            SELECT_SHAPEMETADATA: {
              actions: [...selectFileActions('shapeMetadata'), ...cleanFilesErrorActions()],
