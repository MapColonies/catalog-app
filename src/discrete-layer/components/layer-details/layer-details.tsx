...
export const getValuePresentor = (
  ...
  const fieldTypeName =
    isUiDescriptor(layerRecord) && get(fieldInfo, 'uiDescriptorFieldType');
  ...
  const value =
    formik?.getFieldProps(`${fieldNamePrefix ?? ''}${fieldInfo.fieldName}`)
  ...
  switch (basicType) {
            ...
            <thead>
              <tr>
                <th className="thTableFeature">
                  {intl?.formatMessage({
                    id: 'field-names.vector.featureStructure.fieldName',
                  })}
                </th>
                <th className="thTableFeature">
                  {intl?.formatMessage({
                    id: 'field-names.vector.featureStructure.type',
                  })}
                </th>
              </tr>
            </thead>
    ...
    case 'json':
      return (
        <JsonValuePresentorComponent
          ...
          enableMapPreview={
            enableMapPreview &&
            FOOTPRINT_FIELD_NAMES.includes(fieldName as string)
          }
          enableLoadFromShape={FOOTPRINT_FIELD_NAMES.includes(
            fieldName as string
          )}
          fieldNamePrefix={fieldNamePrefix}
        />
    ...
    case 'links':
      return (
        <LinksValuePresentorComponent
          value={value as LinkModelType[]}
          fieldInfo={fieldInfo}
        />
      );
    case 'url':
      return (
        <UrlValuePresentorComponent
          value={value as string}
          linkInfo={links[(layerRecord as LinkModelType).protocol as LinkType]}
        />
      );
    case 'resolution': {
      const maxFieldRef =
        (fieldInfo.validation as ValidationConfigModelType[])?.filter(
          (valid: ValidationConfigModelType) =>
            valid.valueType === ValidationValueType.FIELD && valid.max
        )[0]?.max || undefined;
      return (
        <ResolutionValuePresentorComponent
          ...
          formik={formik}
          maxFilterValue={
            formik?.getFieldProps(`${fieldNamePrefix ?? ''}${maxFieldRef}`)
              .value
          }
          fieldNamePrefix={fieldNamePrefix}
        />
    ...
    case 'RecordType':
      return (
        <TypeValuePresentorComponent
          value={value as string}
        ></TypeValuePresentorComponent>
      );
    case 'RecordStatus':
      return (
        <StatusValuePresentorComponent
          value={value as string}
        ></StatusValuePresentorComponent>
      );
    ...
    default:
      return (
        <UnknownValuePresentorComponent
          value={basicType}
        ></UnknownValuePresentorComponent>
      );
  }
...
export const LayersDetailsComponent: React.FC<LayersDetailsComponentProps> =
  observer((props: LayersDetailsComponentProps) => {
    const {
      entityDescriptors,
      geoCustomChecks,
      mode,
    ...
    const categoryFieldsParentContainerStyle = (
      CONFIG.NUMBER_OF_CHARACTERS_LIMIT
        ? { [maxLabelLengthCssVar]: `${CONFIG.NUMBER_OF_CHARACTERS_LIMIT}ch` }
        : {}
    ) as React.CSSProperties;

    const renderCategory = (
      category: IRecordCategoryFieldsInfo
    ): JSX.Element => (
      <Box
          ...
          {category.fields
            ?.filter((fieldInfo) => {
              ...
              return (
                (mode !== Mode.NEW &&
                  mode !== Mode.UPDATE &&
                  get(fieldInfo, 'isCreationEssential') !== true) ||
                ((mode === Mode.NEW || mode === Mode.UPDATE) &&
                  get(fieldInfo, 'isAutoGenerated') !== true) ||
                mode === Mode.VIEW ||
                mode === Mode.EDIT ||
                mode === Mode.EXPORT
              );
            })
            .map((fieldInfo: IRecordFieldInfo) => {
              let fieldClassName =
              ...
              const getFilterFieldIdx = (fieldName: string | undefined) => {
                ...
                if (
                  ...
                  layerRecord?.type === RecordType.RECORD_RASTER
                ) {
                  switch (fieldName) {
                ...
                return store.discreteLayersStore.searchParams.catalogFilters.findIndex(
                  (filter) => {
                    return filter.field === aliasFilterField;
                  }
                );
              };
              const stringifyFieldValue = (val: unknown) =>
                (val + '').toLowerCase();

              if (isSearchTab) {
                // Check if catalog filter is enabled by that field
                const isFilterParticipantField =
                  getFilterFieldIdx(fieldInfo.queryableName) > -1;

                const idxFreeTextSearchFilterField =
                  getFilterFieldIdx(PYCSW_ANY_TEXT_FIELD);

                const isByFreeTextSearch =
                  idxFreeTextSearchFilterField > -1 &&
                  stringifyFieldValue(fieldValue).indexOf(
                    stringifyFieldValue(
                      store.discreteLayersStore.searchParams.catalogFilters[
                        idxFreeTextSearchFilterField
                      ].like
                    )
                  ) > -1;
    ...
    const fullInputs = useMemo(() => {
      ...
      return fullArray?.map((category) => {
        category.fields = category.fields?.sort(
    ...
    const briefInputs = useMemo(() => {
      ...
      layerRecord &&
        getEntityDescriptors(layerRecord.__typename, entityDescriptors).forEach(
          (category) => {
            const categoryBriefFields = category.fields?.filter(
              (field) => field.isBriefField
            ) as FieldConfigModelType[];
            briefCategory.fields.push(...categoryBriefFields);
          }
        );

      briefCategory.fields = briefCategory.fields.sort(
        (first, second) => first.isBriefField.order - second.isBriefField.order
      );
      // @ts-ignore
