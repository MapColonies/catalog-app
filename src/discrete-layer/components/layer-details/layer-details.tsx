...
export const getValuePresentor = (
  ...
  const value =
    formik?.getFieldProps(`${fieldNamePrefix ?? ''}${fieldInfo.fieldName}`).value ??
  ...
  switch (basicType) {
    ...
    case 'RecordType':
      return <TypeValuePresentorComponent value={value as string}></TypeValuePresentorComponent>;
    case 'RecordStatus':
      return (
    ...
    default:
      return <UnknownValuePresentorComponent value={basicType}></UnknownValuePresentorComponent>;
  }
};

export const LayersDetailsComponent: React.FC<LayersDetailsComponentProps> = observer(
  (props: LayersDetailsComponentProps) => {
    const {
      entityDescriptors,
      geoCustomChecks,
      mode,
        ...
        <Box className="categoryFieldsContainer">
          {category.fields
            ...
            .map((fieldInfo: IRecordFieldInfo) => {
              let fieldClassName =
                fieldInfo.fullWidth === true ? 'categoryFullWidthField' : 'categoryField';
              ...
              return (
                <Box key={fieldInfo.fieldName as string} className={fieldClassName}>
                  <FieldLabelComponent
                    value={fieldInfo.label}
                    isRequired={
                      !!(fieldInfo.isRequired ?? false) && !(isBrief ?? false) && mode !== Mode.VIEW
                    }
                  />
                  {getValuePresentor(
    ...
    const fullInputs = useMemo(() => {
      const fullArray =
        layerRecord && getEntityDescriptors(layerRecord.__typename, entityDescriptors);
