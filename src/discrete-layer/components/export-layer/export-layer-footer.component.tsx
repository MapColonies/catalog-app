...
const ExportFormValidationErrors: React.FC<{
  ...
  const selectionText = intl.formatMessage({
    id: 'export-layer.selection-index.text',
  });
  const generalFieldsText = intl.formatMessage({
    id: 'export-layer.generalFields.text',
  });
  const importedFileErrorTitle = intl.formatMessage({
    id: 'export-layer.fileError.text',
  });
  const serviceErrorTitle = intl.formatMessage({
    id: 'export-layer.serviceError.text',
  });
  const generalErrorTitle = intl.formatMessage({
    id: 'export-layer.generalError.text',
  });
  ...
  useEffect(() => {
    if (!isEmpty(errors)) {
      const errorsBySelection: Map<string, string[]> = new Map<
        string,
        string[]
      >();
      ...
      Object.entries(errors).forEach(([erroredFieldName, errorMsg]) => {
        ...
        const currentSelectionErrors =
          errorsBySelection.get(selectionKey) ?? [];
        const newSelectionError =
          +selectionIdx > GENERAL_FIELDS_IDX
            ? `${fieldLabel}: ${errorMsg?.message as string}`
            : (errorMsg?.message as string);

        errorsBySelection.set(selectionKey, [
          ...currentSelectionErrors,
          newSelectionError,
        ]);
      });
    ...
    <Box className="validationErrorsContainer">
      {validationErrors.size > NONE &&
        Array.from(validationErrors).map(([key, val], i) => {
          return (
            <Box key={`${key}_${i}`} className="errorSelectionContainer">
              <Typography tag="b" className="errorSelectionText">
                {key}:
              </Typography>
              {val.map((error) => (
                <Typography key={`${error}`} tag="p" className="errorText">
                  {error}
                </Typography>
              ))}
            </Box>
...
const ExportLayerFooter: React.FC<ExportLayerFooterProps> = observer(
  ({ handleTabViewChange, onExportSuccess }) => {
    const { formState, handleSubmit } = useFormContext();
    const { exportStore, actionDispatcherStore } = useStore();
    const intl = useIntl();
    const [insufficientSpaceError, setIsInsufficientSpaceError] = useState<
      string | undefined
    >();
    ...
    const { data: freeDiskSpaceRes, loading: isExportFreeDiskSpaceLoading } =
      useGetFreeDiskSpace();
    ...
    const [exportSizeEstimation, setExportSizeEstimation] = useState<
      number | null
    >();
    ...
    const exportTriggerErrorMsg = get(
      exportTriggerError,
      'response.errors[0].serverResponse.data.message'
    ) as string | undefined;

    useEffect(() => {
      if (typeof exportTriggerErrorMsg !== 'undefined') {
        const issuedSelection = extractJsonObjFromString<Polygon>(
          exportTriggerErrorMsg
        );

        if (typeof issuedSelection !== 'undefined') {
          const issuedSelectionId =
            exportStore.getFeatureIdByGeom(issuedSelection);
    ...
    useEffect(() => {
      if (
        ...
        if (exportSizeEstimation > freeDiskSpaceRes) {
          const insufficientSizeErrorText = intl.formatMessage({
            id: 'export-layer.insufficient-space.error',
          });
    ...
    useEffect(() => {
      if (
        typeof exportTriggerRes !== 'undefined' &&
        typeof exportTriggerRes.jobId !== 'undefined'
      ) {
    ...
    const serviceError = (exportTriggerError as boolean)
      ? {
          [`${SERVICE_ERROR_IDX}_`]: {
            message: exportTriggerErrorMsg as string,
          },
        }
      : {};

    const insufficientSpaceErrorObj = !isEmpty(insufficientSpaceError)
      ? {
          [`${GENERAL_ERROR_IDX}_`]: {
            message: insufficientSpaceError as string,
          },
        }
      : {};
