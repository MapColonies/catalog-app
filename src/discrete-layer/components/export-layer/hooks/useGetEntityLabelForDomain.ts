...
const useGetEntityLabelForDomain = (): ((item: RCesiumEntityProps) => string) => {
  ...
  const layerRecordType = useMemo(
    () =>
      get(enums, `${layerToExport?.productType as string}.parentDomain`) as unknown as RecordType,
    [layerToExport]
  );
      ...
      switch (layerRecordType) {
        case RecordType.RECORD_RASTER:
          ...
          return maxVal && minVal
            ? `${degreesPerPixelToZoomLevel(minVal as number)} - ${degreesPerPixelToZoomLevel(
                maxVal as number
              )}`
            : '';
