...
const StatusPresentor: React.FC<StatusPresentorParams> = ({
  task,
  reactKey = '',
}) => {
  const intl = useIntl();
  ...
  const percentageComponent = () => {
    ...
    if (
      task.counts === NUMBER_OF_TASKS_IN_GROUP &&
  ...
  if (task.status === Status.Failed) {
    ...
    return (
      <Box
        key={reactKey}
        className={`${(task.status as string).toLowerCase()} gridCell`}
      >
        <AutoDirectionBox>
...
const TasksRenderer: React.FC<TasksRendererParams> = observer(
  ({ jobId, productType }) => {
...
export const JobDetailsRenderer: React.FC<ICellRendererParams> = observer(
  (props) => {
    const store = useStore();

    const [propsWithJobParams, setPropsWithJobParams] = useState(props);
    const jobId = (props.data as JobModelType).id.replace(
      DETAILS_ROW_ID_SUFFIX,
      ''
    );
    ...
    return (
      <Box
        key={`${jobId}_${store.jobsStore.reloadDataCounter}`}
      ...
      >
        ...
        <Box className="gridContainer">
          ...
          <TasksRenderer
            productType={
              (props.data as JobModelType).productType as ProductType
            }
            jobId={jobId}
          />
        </Box>
