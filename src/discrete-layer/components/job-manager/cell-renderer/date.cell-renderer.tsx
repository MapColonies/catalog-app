...
export const DateCellRenderer: React.FC<IDateCellRendererParams> = (props) => {
  ...
  const shouldShowTime = useMemo(
    () => (DATE_GRANULARITY as DateGranularityType) === DateGranularityType.DATE_AND_TIME,
    []
  );
  ...
  const isComingSoonClassName = (): string => {
    ...
    if (typeof comingSoonDaysIndication === 'undefined' || diffFromToday > FUTURE_DATE_DIFF_ANCHOR)
      return '';
  ...
  if (
    ...
    return (
      <Tooltip content={dateFormatter(date, true)}>
        ...
        >
          <Box className={isComingSoonClassName()}>{relativeDateFormatter(date)}</Box>
