  ...
  useEffect(() => {
    if (transformedFeature) {
      switch (transformedFeature.geometry.type) {
        case 'LineString': {
          const posArr: IPosition[] = transformedFeature.geometry.coordinates.map((coord) => ({
            longitude: coord[0],
            latitude: coord[1],
          }));
        ...
        case 'Polygon': {
          const posArr: IPosition[][] = transformedFeature.geometry.coordinates.map((ring: any) =>
            ring.map((coord: any) => ({ longitude: coord[0], latitude: coord[1] }))
          );
          setCoordinates(posArr);
          break;
        }
