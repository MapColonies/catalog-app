...
export const PolygonParts: React.FC = observer(() => {
  ...
  const [activeLayer, setActiveLayer] = useState(
    store.discreteLayersStore.polygonPartsLayer
  );
  ...
  useEffect(() => {
    if (
      ...
      JSON.stringify(store.discreteLayersStore.polygonPartsLayer)
    ) {
  ...
  const getFieldLabelTranslation = (fieldName: string): string => {
    ...
    const fieldTuple = polygonPartsFieldLabels.find(
      (fieldTuple) =>
        fieldName.toLocaleLowerCase() === fieldTuple[0]?.toLocaleLowerCase()
    );
    if (fieldTuple) {
  ...
  const isOptionsObjValid = () => {
    return (
      optionsPolygonParts.url !== NOT_VALID &&
      optionsPolygonParts.featureType !== NOT_VALID
    );
  };
  ...
  const buildWFSUrl = (layer: ILayerImage) => {
    ...
    if (layer) {
      const url = layer.links
        ?.find((link) => link.protocol === 'WFS')
        ?.url?.split(/[?#]/)[0];
      if (!url) {
        console.log(
  ...
  const buildFeatureType = (layer: ILayerImage) => {
    if (layer) {
      ...
      featureType += '-';
      featureType +=
        ENUMS[(layer as LayerRasterRecordModelType).productType as string]
          .realValue;
      return featureType;
    } else {
      return NOT_VALID;
  ...
  const handleVisualizationPolygonParts = (
    ...
    const getGeoJsonFromEntity = (
      entity: CesiumCesiumEntity
    ): Polygon | undefined => {
      if (entity.polygon) {
        // Polygon
        ...
        const positions = polygonData.positions.map((position) => {
          const worlPosCartographic =
            CesiumCartographic.fromCartesian(position);
    ...
    const pixelSizeInMeters = (
      ...
      const screenPosition = CesiumSceneTransforms.wgs84ToWindowCoordinates(
        scene,
        position
      );
      ...
      const worldRight = scene.camera.pickEllipsoid(
        screenRight,
        scene.globe.ellipsoid
      );
      const worldBottom = scene.camera.pickEllipsoid(
        screenBottom,
        scene.globe.ellipsoid
      );
    ...
    dataSource?.entities.values.forEach((entity: CesiumCesiumEntity) => {
      if (extent && is2D) {
        try {
          ...
          if (featureClippedPolygon) {
            const labelValue = entity.properties?.label.getValue(
              CesiumJulianDate.now()
            );
      ...
      if (entity.polygon) {
        const partResolution = entity.properties?.resolutionDegree.getValue(
          CesiumJulianDate.now()
        );
      ...
      if (entity.billboard) {
        const worldPos = entity.position?.getValue(
          CesiumJulianDate.now()
        ) as CesiumCartesian3;
        ...
        const correctedCarto = new CesiumCartographic(
          ...
          is2D
            ? 500
            : mapViewer.scene.sampleHeight(
                CesiumCartographic.fromCartesian(worldPos)
              )
        );
        ...
        entity.position =
          correctedCartesian as unknown as CesiumPositionProperty;
    ...
    mapViewer.dataSources.remove(
      mapViewer.dataSources.getByName(labelsCollectionName)[0]
    );
    mapViewer.selectedEntityChanged.removeEventListener(deselectLabelEntities);
    if (is2D) {
      const labelsGeoJsonDataSource = new CesiumGeoJsonDataSource(
        labelsCollectionName
      );
      ...
      labelsGeoJsonDataSource
        ...
        .then((dataSource) => {
          dataSource?.entities.values.forEach((entity: CesiumCesiumEntity) => {
            ...
            entity.billboard = new CesiumCesiumBillboardGraphics({
              image: entity.properties?.label.getValue(CesiumJulianDate.now())
                .dataURL,
              heightReference: CesiumHeightReference.NONE, // Ensures it's not clamped and floats above
              scale: 1.0,
              disableDepthTestDistance: Number.POSITIVE_INFINITY,
              ...
              features: [
                {
                  ...
                  properties: {},
                  geometry: {
                    ...shrinkExtremeCoordinatesInOuterRing(
