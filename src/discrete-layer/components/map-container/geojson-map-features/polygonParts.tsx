...
export const PolygonParts: React.FC = observer(() => {
  ...
  useEffect(() => {
    if (
      JSON.stringify(activeLayer) !== JSON.stringify(store.discreteLayersStore.polygonPartsLayer)
    ) {
  ...
  const getFieldLabelTranslation = (fieldName: string): string => {
    let res = 'N/A';
    const fieldTuple = polygonPartsFieldLabels.find(
      (fieldTuple) => fieldName.toLocaleLowerCase() === fieldTuple[0]?.toLocaleLowerCase()
    );
    if (fieldTuple) {
    ...
    dataSource?.entities.values.forEach((entity: CesiumCesiumEntity) => {
      if (extent && is2D) {
        try {
          ...
          console.log(
            '*** Label placement failed: turf.intersect() failed ***',
            'entity -->',
            entity,
            'extent -->',
            extent
      ...
      if (
        processEntityIds.length > 0 &&
        !processEntityIds.some((validId) => entity.id.startsWith(validId))
      ) {
        return;
      }
      if (entity.polygon) {
        ...
        entity.polygon = new CesiumCesiumPolygonGraphics({
          ...
          material: is2D
            ? CesiumColor.fromCssColorString(partColor).withAlpha(0.2)
            : CesiumColor.fromCssColorString(partColor).withAlpha(0.5),
          outline: true,
      ...
      if (entity.billboard) {
        ...
        const correctedCartesian = CesiumCartesian3.fromRadians(
          correctedCarto.longitude,
          correctedCarto.latitude,
          correctedCarto.height
        );
              ...
              geoJsonDataSouce.entities.values.forEach((item) => {
                if (item.polygon) {
                  item.polygon = new CesiumCesiumPolygonGraphics({
                    ...
                    material: CesiumColor.fromCssColorString(
                      CONFIG.CONTEXT_MENUS.MAP.POLYGON_PARTS_FEATURE_CONFIG.color
                    ),
                    outline: true,
                    outlineColor: CesiumColor.fromCssColorString(
                      CONFIG.CONTEXT_MENUS.MAP.POLYGON_PARTS_FEATURE_CONFIG.outlineColor
                    ),
                    outlineWidth:
                      CONFIG.CONTEXT_MENUS.MAP.POLYGON_PARTS_FEATURE_CONFIG.outlineWidth,
                    height: 10000,
                    perPositionHeight: false,
                  });
                }
