        ...
        for (const iterField of category.fields as FieldConfigModelType[]) {
          ...
          if (field.isFilterable) {
            if (
              ...
              const filterableFieldIdx = allFilterableDescriptorsFields.findIndex(
                (filterableField) => field.fieldName === filterableField.fieldName
              );

              // Prevent fields duplications in final array
              if (filterableFieldIdx > -1) {
                const isCurrentFilterableFieldQueryable =
                  !!allFilterableDescriptorsFields[filterableFieldIdx].queryableName;
