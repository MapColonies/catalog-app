...
export const generateLayerRectangle = (
  ...
  return CesiumRectangle.fromDegrees(
    ...bbox(layer.footprint)
  ) as CesiumRectangle;
};

export const applyFactor = (
...
export const generateFactoredLayerRectangle = (
  ...
  if (
    area({ type: 'Feature', properties: {}, geometry: layer.footprint }) /
      1000000 >
    EARTH_AREA
  ) {
    factor = 0;
...
export const findLayerLink = (
  layer: ILayerImage
): LinkModelType | undefined => {
...
export const getTokenParam = (): string => {
  // eslint-disable-next-line
  const { INJECTION_TYPE, ATTRIBUTE_NAME, TOKEN_VALUE } =
    CONFIG.ACCESS_TOKEN as {
...
export const getLinkUrlWithToken = (
  ...
  if (typeof links !== 'undefined') {
    ...
    const urlWithToken = `${linkUrl ?? ''}${
      linkUrl !== undefined ? getTokenParam() : ''
    }`;
    return !isEmpty(urlWithToken) ? urlWithToken : undefined;
  }
};

export const getLinksArrWithTokens = (
  links: LinkModelType[]
): LinkModelType[] => {
...
export const getWMTSOptions = (
  ...
  if (capability) {
    ...
    const tileMatrixSet = get(
      capability,
      'tileMatrixSet[0]'
    ) as TileMatrixSetModelType; // (!IMPORTANT) derived from raster implementation: there is only ONE surved tile matrix set
