...
const DiscreteLayerView: React.FC = observer(() => {
  ...
  const [jobToOpenRasterEntity, setJobToOpenRasterEntity] = useState<JobModelType | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    ...
    localStore.watchMethods(['setItem', 'removeItem'], undefined, (_method, key) => {
  ...
  const mapSettingsLocale = useMemo(
    () => ({
      ...
      TILE_REQUESTS_OPTIMIZATION_CHECKBOX: intl.formatMessage({ id: 'debug-panel.tile-requests-optimization' }),
  ...
  const onPolygonSelection = (polygon: IDrawingEvent, isPoi = false): void => {
    ...
    const line = lineString([
      [bottomLeftPoint.geometry.coordinates[0], bottomLeftPoint.geometry.coordinates[1]],
      [rightTopPoint.geometry.coordinates[0], rightTopPoint.geometry.coordinates[1]],
    ]);
    ...
    return (
      <Tooltip
        content={intl.formatMessage({
          id: `${!tabsPanelExpanded ? 'action.expand.tooltip' : 'action.collapse.tooltip'}`,
        })}
      >
        <IconButton
          className={className}
    ...
    return (
      <div className="tabHeaderContainer">
        <div
          ...
          style={{ backgroundColor: theme.custom?.GC_TAB_ACTIVE_BACKGROUND as string }}
        >
          <div
            className="tabTitle"
            style={{
        ...
        >
          ...
          >
            ...
            {tabIdx === TabViews.CATALOG && (
              <Tooltip
                content={intl.formatMessage({
                  id: catalogFilter ? 'action.show-all.tooltip' : 'action.filter-nonactive.tooltip',
                })}
              >
                <Icon
                ...
                <MenuSurfaceAnchor id="newContainer">
                  ...
                  <Tooltip content={intl.formatMessage({ id: 'action.operations.tooltip' })}>
                    <IconButton
                      ...
                      onClick={(): void => setIsCreateEntityMenuOpen(!isCreateEntityMenuOpen)}
                    />
                  </Tooltip>
                </MenuSurfaceAnchor>
              )}
      ...
      return layers.reduce((legendsList, cesiumLayer): IMapLegend[] => {
        if (typeof get(cesiumLayer.meta, 'layerRecord.links') !== 'undefined') {
          ...
          const shouldAddLegend =
            typeof legendDoc !== 'undefined' || typeof legendImg !== 'undefined';
  ...
  useEffect(() => {
    if (!!jobToOpenRasterEntity) {
      setIsRasterDialogOpen(true);
    }
  }, [jobToOpenRasterEntity]);
        ...
        <Box className="headerSystemAreaContainer">
          <Tooltip content={intl.formatMessage({ id: 'general.whats-new.tooltip' })}>
            <Box className="position">
              {whatsNewVisitedCount === ZERO && <Box className="badge badge_primary"></Box>}
              <IconButton
          ...
          >
            ...
            >
              ...
              {rect && (
                <FlyTo
                  setRect={setRect}
                  layer={store.discreteLayersStore.selectedLayer as LayerMetadataMixedUnion}
                />
              )}
